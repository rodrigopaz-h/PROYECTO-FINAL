...
export default function Card({producto, agregarAlCarrito}) {
    if (!producto || !producto.id) {
        return null; // O algún otro componente de fallback
    }
  return (
    <div className="w-72 bg-white shadow-md rounded-xl duration-500 hover:scale-105 hover:shadow-xl">
            <Link to={`/producto/${producto.id}`}>
                ...
                <div className="px-4 py-3 w-72">
                    <span className="text-gray-400 mr-3 uppercase text-xs">{producto.procedencia}</span>
                    ...
                    <div className="flex items-center">
                        ...
                        {producto.oldPrice && (
                            <del>
                                <p className="text-sm text-gray-600 cursor-auto ml-2">${producto.oldPrice}</p>
                            </del>
                        )}
                        <div className="ml-auto" onClick={() => agregarAlCarrito(producto.id)}>
